...
helper.updateRelease = function (release) {
    var log = utils.sync;
    try {
        var failed_items = this.collectFailedItems(release.items), _release = {};
...
helper.deleteAssets = function (assetUid, lang, callback) {
    try {
        var _path = lang.assetsPath, paths = [assetUid];
        paths.unshift(_path);
        ...
        helper.deleteAssetFolder(assetFolderPath, function (error, data) {
            ...
            return db.Assets().language(lang.code).Query().toJSON().find().spread(function (_assets) {
                if (_assets && _assets.length) {
                    ...
                    return db.Assets().language(lang.code).Entry().Query().query({_bulk_insert: true, entries: _entries}).update().then(function () {
