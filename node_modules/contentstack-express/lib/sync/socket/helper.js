...
/**
 * Method that fetches releases
 * @param  {string}   url      : API url on which request is made
 * @param  {object}   query    : Query filters on the request made
 * @param  {object}   headers  : Header details for the request
 * @param  {Function} callback : Error first callback

 * @return {object}            : Return request data.releases.items (collection)
 */
utils.release = function (url, query, headers, callback, method) {
	return request({
		url: url,
		method: 'GET',
		...
		if (error)
			...
			return cb(null, data.release.items);
		} else {
			return cb(data);
		}
	});
