        ...
        async.waterfall([
            ...
            try {
                if (!error) {
                    if (!remove) {
                        var upsert = function () {
                            db.ContentType(ctUID).language(lang.code).Entry(entryUID).update(result.entry).then(function (data) {
    ...
    try {
        ...
        async.waterfall([
            ...
            try {
                ...
                return self.updateEntityStatus({isItem: isItem, type: 'Asset', objekt: asset, event: eventType, status: status});
            } catch (error) {
                return self.updateEntityStatus({isItem: isItem, type: 'Asset', objekt: asset, event: eventType, message: helper.message(error), status: 3});
            }
        });
    } catch (error) {
        ...
        async.series([
            ...
            function (error, data) {
                if (error) {
                    ...
                    return self.updateEntityStatus({isItem: false, objekt: form, status: 4, event: 'deleting', type: 'Content type'});
                }
            });
    } catch (error) {
        ...
        async
            .series([
                ...
                function (error, data) {
                    ...
                    return self.updateEntityStatus({isItem: false, objekt: folder, status: 4, event: 'deleting', type: 'Folder'});
                });
    } catch (error) {
