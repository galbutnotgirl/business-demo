...
function Config() {
  try {
    var env = process.env.NODE_ENV || "development",
      ...
      _dirname = process.env.SITE_PATH
        ? path.resolve(process.env.SITE_PATH)
        : process.cwd();

    var _path = path.join(_dirname, "config"),
      _default = require("./default"),
      _all = fs.existsSync(path.join(_path, "all.js"))
        ? require(path.join(_path, "all"))
        : {},
      _env = fs.existsSync(path.join(_path, env + ".js"))
        ? require(path.join(_path, env))
        : {};
    ...
    _config.path = {
      ...
      favicon: path.isAbsolute(_config.static.path)
        ? path.resolve(_config.static.path, "favicon.ico")
        : path.resolve(_theme, _config.static.path, "favicon.ico"),
      logs: path.isAbsolute(_config.logs.path)
        ? _config.logs.path
        : path.join(_dirname, _config.logs.path),
      storage:
        _config.storage &&
        _config.storage.options &&
        path.isAbsolute(_config.storage.options.basedir)
          ? _config.storage.options.basedir
          : path.join(
              _dirname,
              _config.storage && _config.storage.options
                ? _config.storage.options.basedir || "./_content"
                : "./_content"
            )
    };
    ...
    if (_config.view.custom_templates) {
      if (
        typeof _config.view.custom_templates === "string" &&
        fs.existsSync(path.join(_dirname, _config.view.custom_templates))
      ) {
        _config.path.templates.push(
          path.join(_dirname, _config.view.custom_templates)
        );
      } else if (
        typeof _config.view.custom_templates === "object" &&
        _config.view.custom_templates instanceof Array
      ) {
        _config.view.custom_templates.forEach(function(template_path) {
          if (fs.existsSync(_dirname, template_path))
            ...
            console.error(
              template_path + " provided in config failed to resolve."
            );
        });
      } else {
        console.error(
          _config.view.custom_templates +
            " provided in config failed to resolve."
        );
      }
    }
    ...
    if (
      _config.storage &&
      _config.storage.provider.toLowerCase() !== "filesystem"
    ) {
      _config.path.assets =
        _config.assets && path.isAbsolute(_config.assets.basedir)
          ? _config.assets.basedir
          : path.join(_dirname, _config.assets.basedir || "./assets");
    }
    // configure storage
